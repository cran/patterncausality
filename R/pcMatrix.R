...
#' @description Analyzes pattern causality relationships between multiple
#'   time series
...
#' @param metric Character; distance metric ("euclidean", "manhattan",
#'   "maximum")
...
#'   if FALSE calculates absolute changes (new-old) in signature space.
#'   Default is TRUE.
...
pcMatrix <- function(dataset, E, tau, metric = "euclidean", h, weighted = TRUE,
  ...
  if (!is.character(metric) ||
      !metric %in% c("euclidean", "manhattan", "maximum")) {
    stop("metric must be one of: 'euclidean', 'manhattan', 'maximum'",
         call. = FALSE)
  }
  ...
  if (n_cores > 1) {
    ...
    parallel::clusterExport(cl, c(
      "dataset", "E", "tau", "metric", "h", "weighted",
      "distance_fn", "state_space_fn", "relative", "pcLightweight"
    ), envir = environment())
    ...
    results <- parallel::parLapply(cl, seq_len(nrow(grid)), function(idx) {
      i <- grid$i[idx]
      j <- grid$j[idx]
