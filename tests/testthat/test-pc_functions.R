...
test_that("pcLightweight handles errors appropriately", {
  ...
  expect_error(pcLightweight(X, Y, E = 1, tau = 2, metric = "euclidean", h = 1, weighted = TRUE))  # E must be > 1
...
# Test E parameter validation in core functions
test_that("E parameter validation works correctly", {
  data(climate_indices)
  X <- climate_indices$AO[1:50]

  # Test stateSpace function
  expect_error(stateSpace(X, E = 1, tau = 1), "E must be an integer greater than 1")
  expect_no_error(stateSpace(X, E = 2, tau = 1))

  # Test stateSpaceMethod function
  expect_error(stateSpaceMethod(X, E = 1, tau = 1), "E must be an integer greater than 1")
  expect_no_error(stateSpaceMethod(X, E = 2, tau = 1))
})
